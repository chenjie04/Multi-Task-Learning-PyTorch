...
def get_backbone(p):
    ...
    if p["backbone"] == "resnet18":
        ...
        backbone = SquadNet(
            ...
            depths=p["backbone_kwargs"]["depths"],
            channel_shuffle=p["backbone_kwargs"]["channel_shuffle"],
        )
        backbone_channels = (
            p["backbone_kwargs"]["channels"]
            * int(math.pow(2, (3 - 1)))
            * (p["model_kwargs"]["num_tasks"] + p["model_kwargs"]["num_shared"])
        )
    ...
    if (
        "fuse_hrnet" in p["backbone_kwargs"] and p["backbone_kwargs"]["fuse_hrnet"]
    ...
    if p["setup"] == "single_task":
        ...
        if p["model"] == "baseline":
            ...
            heads = torch.nn.ModuleDict(
                {task: get_head(p, backbone_channels, task) for task in p.TASKS.NAMES}
            )
            ...
            for task in p.TASKS.NAMES:
                model = SingleTaskModel(
                    copy.deepcopy(backbone), get_head(p, backbone_channels, task), task
                )
            ...
            model = CrossStitchNetwork(
                ...
                torch.nn.ModuleDict(decoder_dict),
...
def get_transformations(p):
    ...
    transforms_tr.extend(
        [
            tr.FixedResize(
                resolutions={x: tuple(p.TRAIN.SCALE) for x in p.ALL_TASKS.FLAGVALS},
                flagvals={x: p.ALL_TASKS.FLAGVALS[x] for x in p.ALL_TASKS.FLAGVALS},
            )
        ]
    )
    ...
    transforms_ts.extend(
        [
            tr.FixedResize(
                resolutions={x: tuple(p.TEST.SCALE) for x in p.TASKS.FLAGVALS},
...
def get_loss(p, task=None):
    ...
    if task == "edge":
        ...
        raise NotImplementedError(
            "Undefined Loss: Choose a task among "
    ...
    if p["setup"] == "single_task":
        ...
        if p["loss_kwargs"]["loss_scheme"] == "baseline":  # Fixed weights
            ...
            loss_ft = torch.nn.ModuleDict(
                {task: get_loss(p, task) for task in p.TASKS.NAMES}
            )
            loss_weights = p["loss_kwargs"]["loss_weights"]
            return MultiTaskLoss(p.TASKS.NAMES, loss_ft, loss_weights)

        elif (
            p["loss_kwargs"]["loss_scheme"] == "pad_net"
        ):  # Fixed weights but w/ deep supervision
            from losses.loss_schemes import PADNetLoss

            loss_ft = torch.nn.ModuleDict(
                {task: get_loss(p, task) for task in p.ALL_TASKS.NAMES}
            )
            loss_weights = p["loss_kwargs"]["loss_weights"]
            return PADNetLoss(
                p.TASKS.NAMES, p.AUXILARY_TASKS.NAMES, loss_ft, loss_weights
            )

        elif (
            p["loss_kwargs"]["loss_scheme"] == "mti_net"
        ):  # Fixed weights but at multiple scales
            from losses.loss_schemes import MTINetLoss

            loss_ft = torch.nn.ModuleDict(
                {task: get_loss(p, task) for task in set(p.ALL_TASKS.NAMES)}
            )
            loss_weights = p["loss_kwargs"]["loss_weights"]
            return MTINetLoss(
                p.TASKS.NAMES, p.AUXILARY_TASKS.NAMES, loss_ft, loss_weights
            ...
            raise NotImplementedError(
                "Unknown loss scheme {}".format(p["loss_kwargs"]["loss_scheme"])
            )
...
def get_optimizer(p, model):
    ...
    if p["model"] == "cross_stitch":  # Custom learning rate for cross-stitch
        ...
        cross_stitch_params = [
            param for name, param in model.named_parameters() if "cross_stitch" in name
        ]
        single_task_params = [
            param
            for name, param in model.named_parameters()
            if not "cross_stitch" in name
        ]
        assert p["optimizer"] == "sgd"  # Adam seems to fail for cross-stitch nets
        optimizer = torch.optim.SGD(
            [
                {
                    "params": cross_stitch_params,
                    "lr": 100 * p["optimizer_kwargs"]["lr"],
                },
        ...
        nddr_params = [
            param for name, param in model.named_parameters() if "nddr" in name
        ]
        single_task_params = [
            param for name, param in model.named_parameters() if not "nddr" in name
        ]
        assert p["optimizer"] == "sgd"  # Adam seems to fail for nddr-cnns
        optimizer = torch.optim.SGD(
            [
                {"params": nddr_params, "lr": 100 * p["optimizer_kwargs"]["lr"]},
                {"params": single_task_params, "lr": p["optimizer_kwargs"]["lr"]},
            ],
            momentum=p["optimizer_kwargs"]["momentum"],
            nesterov=p["optimizer_kwargs"]["nesterov"],
...
def adjust_learning_rate(p, optimizer, epoch):
    ...
    if p["scheduler"] == "step":
        ...
        if lr > 1e-5:
            # lr = lr * (1 - epoch/p['epochs']) ** 0.9
            lambd = pow(1 - (epoch / p["epochs"]), 0.99)
            lr = lr * lambd
