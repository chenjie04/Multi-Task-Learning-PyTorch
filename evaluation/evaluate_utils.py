...
def validate_results(p, current, reference):
    ...
    if len(tasks) == 1: # Single-task performance
        ...
        if current['multi_task_performance'] > reference['multi_task_performance']:
            print('Has New best multi-task model %.2f -> %.2f' %(100*reference['multi_task_performance'], 100*current['multi_task_performance']))
            improvement = True
    ...
    for sample in tqdm(val_loader):       
        ...
        for task in p.TASKS.NAMES:
            ...
            for jj in range(int(inputs.size()[0])):
                ...
                result = cv2.resize(output_task[jj], dsize=(int(meta['im_size'][1][jj].numpy()), int(meta['im_size'][0][jj].numpy())), interpolation=p.TASKS.INFER_FLAGVALS[task])
