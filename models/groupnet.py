...
class SeparableGroupConvModule(BaseModule):
    def __init__(
        self,
        channels: int = 32,
        num_tasks: int = 5,
    ...
    def forward(self, x):
        shortcut = x
        input_list = torch.split(x, self.channels, dim=1)
        shared_features = torch.cat(input_list[-self.num_shared :], dim=1)
        output_list = []
        for i in range(self.num_tasks):
            output_list.append(self.conv_list[i](input_list[i]))
        x = torch.cat(output_list, dim=1)
        x = torch.cat([x, shared_features], dim=1)
        if self.channel_shuffle:
